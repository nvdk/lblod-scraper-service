...
from sudo_query import auth_update_sudo, update_sudo

from constants import SCRAPE_JOB_TYPE, RESOURCE_BASE, EXTRACTION_GRAPH
...
def run_job(job_uri, graph, runner_func, sparql_query, sparql_update):
    ...
    if job_res["results"]["bindings"]:
        used = [binding["used"]["value"] for binding in job_res["results"]["bindings"]]
    else:
        raise Exception(f"Didn't find actionable job for <{job_uri}>")

    # logger.info(f"Started running job {job_uri} at {start_time}")
...
def link_job_to_parent(job, parent_job):
    query_template = Template("""
    PREFIX prov: <http://www.w3.org/ns/prov#>
    INSERT DATA {
    GRAPH $graph {
    $job prov:wasInformedBy $parent_job
    }
}""")
    query_string = query_template.substitute(
        graph=sparql_escape_uri(EXTRACTION_GRAPH),
        job=sparql_escape_uri(job["uri"]),
        parent_job=sparql_escape_uri(parent_job["uri"]),
    )
    update_sudo(query_string)

###
# Service specific functions, not generic, shouldn't be placed in package
###
def create_scrape_job(source_url, parent_job=None):
    (q_create, job) = create_job(SCRAPE_JOB_TYPE, RESOURCE_BASE, EXTRACTION_GRAPH)
    update_sudo(q_create)
    if (parent_job):
        link_job_to_parent(job, parent_job)
    q_update_status = update_job_status(job["uri"], STATUS_STARTED, EXTRACTION_GRAPH)
    update_sudo(q_update_status)
    q_attach_sources = attach_job_sources(job["uri"], [source_url], EXTRACTION_GRAPH)
    update_sudo(q_attach_sources)
    logger.info(f"Running scraping job <{job['uri']}> with source <{source_url}>")
    return job
